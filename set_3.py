...
def tic_tac_toe(board):
    ...
    for i in range(x):
        if board[i][0] != "" and all(board[i][j] == board [i][0] for j in range (x)):
            return board[i][0]
        
    for j in range(x):
        if board [0][j] != "" and all(board[i][j] == board[0][j] for i in range(x)):
            return board[0][j]
        
    if board[i][i] != "" and all(board[i][i] == board[0][0] for i in range(x)):
        return board[i][i]
    
    if board[0][x-1] !="" and all(board[i][x-i-1] == board[0][x-1] for i in range(x)):
        return board[0][x-1]
    ...
    while True:
        for stop in route_map:
            if stop[0] == current_stop:
                side_stop = stop[1]
                travel_time += route_map[stop[0],stop[1]]["travel_time_mins"]
                current_stop = side_stop
